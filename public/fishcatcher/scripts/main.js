...
'use strict'; {
    ...
    window.RuntimeInterface = class RuntimeInterface {
        constructor(opts) {
            ...
            if (this._useWorker && (typeof OffscreenCanvas === "undefined" || !navigator["userActivation"] || this._scriptsType === "module" && !SupportsWorkerTypeModule())) this._useWorker = false;
        ...
        _GetWorkerURL(url) {
            ...
            else if (this._exportType === "playable-ad" &&
                this._localFileBlobs.hasOwnProperty(url.toLowerCase())) ret = this._localFileBlobs[url.toLowerCase()];
            else ret = url;
        ...
        async _InitWorker(opts, port2) {
            const workerMainUrl =
                this._GetWorkerURL(opts.workerMainUrl);
            ...
            if (this._exportType ===
                "preview") self["goToLastErrorScript"] = () => this.PostToRuntimeComponent("runtime", "go-to-last-error-script")
        }
        async _InitDOM(opts, port2) {
            ...
            } else if (opts.projectScripts &&
                opts.projectScripts.length > 0) try {
